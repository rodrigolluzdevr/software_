...
export default function DirectorsList() {
    const router = useRouter();
      ...
      // Filter users to only include those with ROLE="DIRETOR"
      const directors = allUsers.filter((user) => user.role === 'DIRETOR');
      ...
      // Obter apenas os diretores para a página atual
      ...
      function handleRegisterClick() {
        router.push('/users/directors/register');
      }
    
      function handleEditDirector(userId: string | number) {
        router.push(`/users/directors/update/${userId}`);
      }
        ...
        <section className="w-full px-2 sm:px-3 md:px-4 lg:px-6">
          <div className="layout-specing">
            ...
            {!isLoading && !error && (
              <>
                <SearchBar
                  placeholder="Pesquisar diretores..."
                  value={searchTerm}
