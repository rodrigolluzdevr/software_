...
const SchoolClassUpdate = () => {
  ...
  // Load class data when component mounts and id is available
  useEffect(() => {
    ...
    const fetchClassData = async () => {
      setLoading(true);
      try {
        console.log(`Buscando dados para turma com id: ${id}`);
        ...
        if (!response.ok) {
          ...
          throw new Error(`Falha ao buscar dados da turma (${response.status})`);
        }
        
        const data: ClassData = await response.json();
        console.log('Dados da turma recebidos:', data);
        setDebugInfo(`Dados carregados com sucesso para turma: ${data.name}`);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError('');
    setLoading(true);
    setDebugInfo(`Enviando atualização para turma id: ${id}...`);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let errorMessage = 'Falha na atualização da turma, tente novamente';
        try {
        ...
        if (response.status === 401) {
          ...
          setError('Você não tem permissão para atualizar esta turma');
        } else if (response.status === 404) {
          setError('Turma não encontrada');
        } else {
          ...
          <div className="w-full px-2 sm:px-3 md:px-4 lg:px-6">
            <div className="py-4">
              <div className="shadow-sm rounded bg-white">
                <div className="p-5">
                  <h5 className="text-lg font-semibold">Editar Turma</h5>
                </div>
...
export default withAuth(SchoolClassUpdate, ['DIRETOR']);