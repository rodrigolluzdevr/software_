...
const CoordinatorUpdate = () => {
  ...
  const { personalInfo, address, workInfo } = formData;
  
  // debug state to help track issues
  const [debugInfo, setDebugInfo] = useState<string>('');
  
  // load coordinator data when component mounts and id is available
  useEffect(() => {
    ...
    const fetchCoordinatorData = async () => {
      setLoading(true);
      try {
        console.log(`fetching data for coordinator id: ${id}`);
        ...
        if (!response.ok) {
          ...
          throw new Error(`failed to fetch coordinator data (${response.status})`);
        }
        
        const data: CoordinatorData = await response.json();
        console.log('coordinator data received:', data);
        console.log('coordinator isActive status:', data.isActive);
        setDebugInfo(`data loaded successfully for coordinator: ${data.name}`);
        ...
        if (!(error instanceof Error && error.message.includes('401'))) {
          setError('error loading coordinator data');
        }
      } finally {
        setLoading(false);
      }
  ...
  // load organization id from token if not set from coordinator data
  useEffect(() => {
  ...
  const updateWorkInfo = (field: string, value: string | boolean) => {
  ...
  const handleActiveStatusChange = (newValue: boolean) => {
    const currentStatus = workInfo.isActive;
    ...
    setFormData(prevState => {
      ...
      console.log('new isActive state after update:', updatedData.workInfo.isActive);
  ...
  // form submission handler for updating coordinator
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError('');
    setLoading(true);
    setDebugInfo(`submitting update for coordinator id: ${id}...`);

    try {
      const numericCpf = getNumericValue(personalInfo.cpf);
      const numericCep = getNumericValue(address.cep);
      
      console.log('isActive value before building payload:', workInfo.isActive);
      ...
      const requestBody = {
        ...
        organizationId,
        isActive: Boolean(workInfo.isActive),
        birthDate
      };
      ...
      if (!response.ok) {
        ...
        let errorMessage = 'falha na atualização do coordenador, tente novamente';
        try {
        ...
        if (response.status === 401) {
          ...
          setError('você não tem permissão para atualizar este coordenador');
        } else if (response.status === 404) {
      ...
      router.push('/users/coordinators/');
    } catch (err: any) {
  ...
  const breadcrumbItems = [
    ...
    { label: 'Coordenadores', href: '/users/coordinators' },
                      ...
                      <div className="grid grid-cols-6 gap-6 mb-6">
                        <div className="col-span-6 sm:col-span-1 lg:col-span-1 flex items-end">
                          <div className="flex items-center">
                            <ToggleSwitch
                              label="Status"
                              checked={workInfo.isActive}
                              onChange={handleActiveStatusChange}
                            />
                            {process.env.NODE_ENV !== 'production' && (
                              <span className="ml-2 text-xs text-gray-500">
                                {workInfo.isActive ? 'Ativo' : 'Inativo'}
                              </span>
                            )}
                          </div>
                        </div>
...
export default withAuth(CoordinatorUpdate, ['SECRETARIO']);