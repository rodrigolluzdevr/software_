...
const DirectorUpdate = () => {
  ...
  // load director data when component mounts and id is available
  useEffect(() => {
    ...
    const fetchDirectorData = async () => {
      setLoading(true);
      try {
        console.log(`fetching data for director id: ${id}`);
        ...
        if (!response.ok) {
          ...
          throw new Error(`failed to fetch director data (${response.status})`);
        }
        
        const data: DirectorData = await response.json();
        console.log('director data received:', data);
        console.log('director isActive status:', data.isActive);
        setDebugInfo(`data loaded successfully for director: ${data.name}`);
        ...
        if (!(error instanceof Error && error.message.includes('401'))) {
          setError('error loading director data');
        }
      } finally {
        setLoading(false);
      }
  ...
  // load organization id from token if not set from director data
  useEffect(() => {
  ...
  // form submission handler for updating director
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError('');
    setLoading(true);
    setDebugInfo(`submitting update for director id: ${id}...`);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let errorMessage = 'falha na atualização do diretor, tente novamente';
        try {
        ...
        if (response.status === 401) {
          ...
          setError('você não tem permissão para atualizar este diretor');
        } else if (response.status === 404) {
          setError('diretor não encontrado');
        } else {
  ...
  const breadcrumbItems = [
    ...
    { label: 'Diretores', href: '/users/directors' },
...
export default withAuth(DirectorUpdate, ['SECRETARIO', 'COORDENADOR']);