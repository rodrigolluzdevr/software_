...
const RegionUpdate = () => {
  ...
  // Load region data when component mounts and id is available
  useEffect(() => {
    ...
    const fetchRegionData = async () => {
      setLoading(true);
      try {
        console.log(`Buscando dados para região com id: ${id}`);
        ...
        // Fetch region data
        const response = await fetch(`http://localhost:4000/regions/${id}`, {
          headers: {
        ...
        if (!response.ok) {
          ...
          if (response.status === 401) {
            ...
            setError('Região não encontrada');
          } else {
            setError(`Erro ao carregar dados (${response.status})`);
          }
          
          throw new Error(`Falha ao buscar dados da região (${response.status})`);
        }
        
        const data: RegionData = await response.json();
        console.log('Dados da região recebidos:', data);
        setDebugInfo(`Dados carregados com sucesso para região: ${data.name}`);
        ...
        console.error('Erro ao buscar dados da região:', error);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError('');
    setLoading(true);
    setDebugInfo(`Enviando atualização para região id: ${id}...`);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let errorMessage = 'Falha na atualização da região, tente novamente';
        try {
        ...
        if (response.status === 401) {
          ...
          setError('Você não tem permissão para atualizar esta região');
        } else if (response.status === 404) {
          setError('Região não encontrada');
        } else {
          ...
          <div className="w-full px-2 sm:px-3 md:px-4 lg:px-6">
            <div className="py-4">
              <div className="shadow-sm rounded bg-white">
                <div className="p-5">
                  <h5 className="text-lg font-semibold">Editar Região</h5>
                </div>
                <div className="p-5 border-t border-gray-100">
                  ...
                  {loading && !error ? (
                    <div className="text-center py-8">
                      <p>Carregando dados da região...</p>
                    </div>
                  ) : error ? (
...
export default withAuth(RegionUpdate, ['SECRETARIO']);